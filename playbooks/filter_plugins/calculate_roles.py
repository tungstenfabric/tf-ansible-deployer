...
class FilterModule(object):
    ...
    def calculate_openstack_roles(self, existing_dict, instances_dict, global_configuration,
        ...
        instances, deleted_ips = self.os_roles.discover_openstack_roles(hv)
    ...
    def calculate_contrail_roles(self, existing_dict, api_server_list,
        ...
        instances, deleted_ips, api_server_ip = self.contrail_roles.discover_contrail_roles()
    ...
    def calculate_deleted_toragent_roles(self, cluster_roles_dict, instance_data):
        ...
        deleted_ips = cluster_roles_dict['deleted_ips']
        instance_roles = []
...
class OpenstackCluster(object):
    ...
    def discover_openstack_roles(self, hv):
        ...
        try:
            instances, deleted_ips = \
                self.discover_openstack_computes(instances, deleted_ips, hv)
        except Exception:
            return dict(), dict()
        ...
        for server in instances:
            ...
            if len(instances[server]['deleted_roles']) and\
                    (instances[server]['deleted_roles'] ==
                        instances[server]['existing_roles']):
                deleted_ips[server] = self.node_name_ip_map[server]

        self.node_roles_dict = instances
        self.deleted_ips = deleted_ips
        return instances, deleted_ips
    ...
    def discover_openstack_computes(self, instances, deleted_ips, hv):
        ...
        for hyp in response["hypervisors"]:
            ...
            if server_name:
                ...
                deleted_ips[hostname] = self.instances_dict[hostname]['ip']

        return instances, deleted_ips

    def discover_openstack_controllers(self, instances, deleted_ips):
        ...
        for endpoint in response["endpoints"]:
            ...
            if ip in self.node_ip_name_map:
                ...
                deleted_ips[deleted_server_name] = ip
        return instances, deleted_ips
...
class ContrailCluster(object):
    ...
    def discover_contrail_cluster(self, instances, deleted_ips):
    ...
    def discover_contrail_roles(self):
        ...
        if self.existing_tor_agents:
            for toragent in self.existing_tor_agents:
                ...
                elif server not in deleted_ips:
                    deleted_ips[server] = self.existing_tor_agents[toragent]['toragent_ip']

        return instances, deleted_ips, self.api_server_ip
